Ось оновлений варіант вашого `README.md` англійською мовою. Я замінив «Internet Acquiring» на **«Payment Processing System»** — це звучить більш професійно та універсально для розробника, уникаючи специфічного терміну, який ви хотіли прибрати.

---

# 👋 Hi, I'm Artem!

## 🚀 About Me

Frontend Developer with 3.5+ years of experience in building scalable and high-performance web applications. I specialize in **React** and **TypeScript** ecosystems with a strong focus on performance, clean architecture, and system reliability. I have a proven track record of delivering production-grade solutions in **Fintech** and **Retail** domains.

I am also a **Programming Instructor**, helping developers grow through structured learning and mentorship.

---

## 💼 Professional Experience

* **Frontend Developer @ TenThousand** *(Apr 2026 — Current)*
* **Frontend Developer @ InStandart** *(Sep 2024 — Apr 2026)*
* **Programming Instructor @ IT Generation** *(Jan 2024 — Current)*
* **Frontend Developer @ MB-Digital** *(Sep 2023 — Mar 2024)*

---

## 🛠️ Tech Stack

* **Frontend:** JavaScript (ES6+), TypeScript, React, Redux (RTK), Zustand, TanStack Query
* **UI & Styling:** Tailwind CSS, Material UI, Ant Design, Shadcn/ui, Framer Motion
* **Backend Integration:** Node.js (Express), NestJS, PostgreSQL (TypeORM/Prisma), REST API
* **Tools & Environment:** Vite, Git, Docker, Linux, Figma

---

## 🌟 Featured Projects

### 📊 CRM for Educational Business (Team Lead)

*Comprehensive Dockerized system for managing educational workflows.*

* **Role:** Team Lead. Responsible for system architecture, conducting code reviews, task management, CI/CD implementation, and team mentorship.
* **Stack:** Laravel 11 (API) + React (SPA), PostgreSQL, Reverb (WebSockets).
* **Key Features:** Full educational lifecycle management, real-time chat, automated Telegram notifications, and profit analytics.

### 💳 Payment Processing Backend System

*Backend service simulating a secure payment management lifecycle.*

* **Stack:** NestJS, PostgreSQL, TypeScript.
* **Key Features:** Implementation of complex payment session workflows (creation, authorization, confirmation, partial/full refunds, and cancellations).
* **Architecture:** Developed a robust state machine logic, handled edge cases (timeouts, API failures), implemented detailed transaction history tracking, and achieved high reliability through unit testing.

---

## 🧠 Leadership & Processes

As a Team Lead, I believe that quality products are built through effective processes:

* **Code Quality:** Ensuring high standards of code readability, maintainability, and adherence to SOLID/DRY principles through rigorous code reviews.
* **Task Management:** Breaking down complex features into actionable tasks, prioritizing backlogs, and ensuring timely delivery.
* **Mentorship:** Providing constructive feedback, resolving technical blockers, and fostering the professional growth of team members.
* **Communication:** Bridging the gap between business requirements and technical implementation.

---

## 🎯 Philosophy & Focus

* ⚡ **Performance Engineering** — Building fast, scalable, and efficient interfaces.
* 🛠 **Clean Architecture** — Reducing technical debt through maintainable solutions.
* 🎓 **Mentorship** — Empowering developers to achieve their full potential.

---

## 🤝 Connect With Me

* **Email:** [kotartemmodestayn@gmail.com](https://www.google.com/search?q=mailto%3Akotartemmodestayn%40gmail.com)
* **GitHub:** [github.com/Modestayn](https://www.google.com/search?q=https://github.com/Modestayn)
* **Telegram:** [@Modestayn](https://www.google.com/search?q=https%3A%2F%2Ft.me%2FModestayn)
* **Website:** [personal-weebsite-kotartem.netlify.app](https://www.google.com/search?q=https%3A%2F%2Fpersonal-weebsite-kotartem.netlify.app%2F)
